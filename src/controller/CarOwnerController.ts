...
@JsonController()
export default class CarOwnerController {
	private CarOwnerRepo = AppDataSource.getRepository(CarOwner) // Car Repository
	...
	async read(@Req() req: Request, @Res() res: Response) {
		if (req.params.id) return this.CarOwnerRepo.findOne({where: {id: req.params.id}} )
		else {
			const searchString = req.query.params
			const findOptions: any = {order: {}, where: []}
			const existingFields = this.CarOwnerRepo.metadata.ownColumns.map((col) => col.propertyName)
			...
			return this.CarOwnerRepo.find(findOptions)
		}
	}
	...
	async delete(@Req() req: Request, @Res() res: Response) {
		...
		{
			const CarOwnerToRemove = await this.CarOwnerRepo.findOne(  {where: {id: req.params.id}})
			res.statusCode = 204
			if (CarOwnerToRemove) return this.CarOwnerRepo.remove(CarOwnerToRemove)
			else{
	...
	async update(@Body() reqBody: any, @Param('confirmid') confirmid: number, @Res() res: Response, @Req() req: Request) {
		...
		if(token == 'Bearer admin')
		{
			const CarOwnerToUpdate = await this.CarOwnerRepo.preload(reqBody)
			if (!CarOwnerToUpdate || CarOwnerToUpdate.id != confirmid){
				// do nothing
			}
			else {
				const violations = await validate(CarOwnerToUpdate, this.validOptions)
				if (violations.length) {
					res.statusCode = 422 // Unprocessable Entity
					return violations
				} else {
					return this.CarOwnerRepo.save(CarOwnerToUpdate)
				}
			}
			//They do not have access return a message
		} else{
	...
	async create(@Body() reqBody:any,@Res() res:Response, @Req() req: Request) {
		...
		//If token is Bearer admin then they can Update then entry in the DB
		if (token == 'Bearer admin') {
			const CarOwnerToCreate = Object.assign(new CarOwner(), reqBody)
			const violations = await validate(CarOwnerToCreate, this.validOptions)
			if (violations.length) {
				res.statusCode = 422 // Unprocessable Entity
				return violations
			} else {
				return this.CarOwnerRepo.save(CarOwnerToCreate)
			}
		}
		//They do not have access return a message
		else
